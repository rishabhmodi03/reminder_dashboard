...
import { Save, FileText, CheckCircle2, FilePenLine, Clock } from "lucide-react";
...
export default function DailyLog() {
    ...
    useEffect(() => {
        if (todayLog && !todayText && !isSaving) {
            setTodayText(todayLog.entries.join('\n'));
            if (todayLog.studyHours) setStudyHours(todayLog.studyHours);
        }
    }, [todayLog, loading]); // eslint-disable-line react-hooks/exhaustive-deps

    const handleSave = async () => {
        setIsSaving(true);
        const currentTime = format(new Date(), 'hh:mm a');

        let currentLines = todayText.split('\n');
        ...
        const oldLines = todayLog ? todayLog.entries : [];
        let matches = true;
        ...
        } else if (!todayLog && currentLines.length > 0) {
        ...
        setTodayText(currentLines.join('\n'));
    ...
    const pastLogs = logs.filter(l => l.date !== today).slice(0, 5);
        ...
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <section className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-8 rounded-[2rem] shadow-[0_0_50px_-15px_rgba(99,102,241,0.1)] h-fit relative">
                    ...
                    <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white bg-clip-text text-transparent bg-gradient-to-r from-gray-100 to-gray-400">
